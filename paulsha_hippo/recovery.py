...
def _authority_sources(
    memory_root: Path, manifest_path: str | Path
) -> tuple[list[Path], dict[str, str], dict[str, Any]]:
    """Reuse an earlier recovery manifest's exact frozen source authority.

    A live archive keeps receiving captures while recovery is running.  Replanning
    from the entire current archive can therefore pull an active session into the
    transaction and race its importer target forever.  The prior manifest is the
    reviewed scope authority; code/config/source hashes and planned artifacts are
    still regenerated by the currently installed candidate.
    """
    authority = Path(manifest_path).resolve()
    try:
        raw = authority.read_bytes()
        payload = json.loads(raw.decode("utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise RecoveryError(f"cannot read source authority manifest: {exc}") from exc
    if not isinstance(payload, dict) or payload.get("schema_version") != SCHEMA_VERSION:
        raise RecoveryError("unsupported source authority manifest")
    if Path(str(payload.get("memory_root") or "")).resolve() != memory_root:
        raise RecoveryError("source authority memory_root mismatch")
    entries = payload.get("entries")
    if not isinstance(entries, list):
        raise RecoveryError("source authority manifest has no entries")

    archive_root = (memory_root / "archive" / "queue").resolve()
    source_sets: dict[str, str] = {}
    for entry in entries:
        if not isinstance(entry, dict):
            raise RecoveryError("source authority entry must be an object")
    ...
    for source in sources:
        if not source.is_file():
            raise RecoveryError(f"source authority file is missing: {source}")
    return sources, source_sets, {
        "mode": "authority-manifest",
        "reference_manifest": str(authority),
        "reference_manifest_sha256": _sha_bytes(raw),
        "reference_recovery_id": payload.get("recovery_id"),
...
def create_plan(
    ...
    *,
    ...
    source_scope: dict[str, Any] = {"mode": "current-archive"}
    authority_sets: dict[str, str] | None = None
    if source_manifest_path is not None:
        sources, authority_sets, source_scope = _authority_sources(
            root, source_manifest_path
        )
        authority_baseline_count = sum(
            source_set == "baseline" for source_set in authority_sets.values()
        )
        if baseline_count is not None and baseline_count != authority_baseline_count:
            raise RecoveryError(
                "baseline_count disagrees with source authority manifest: "
                f"expected {authority_baseline_count}, got {baseline_count}"
            )
        baseline_count = authority_baseline_count
        baseline_sources = {
            Path(source) for source, source_set in authority_sets.items()
...
def _verify_pins(manifest: dict[str, Any]) -> Path:
    ...
    if isinstance(source_scope, dict) and source_scope.get("mode") == "authority-manifest":
        authority = Path(str(source_scope.get("reference_manifest") or "")).resolve()
        expected_authority_hash = str(
            source_scope.get("reference_manifest_sha256") or ""
        )
        if not authority.is_file() or _sha_path(authority) != expected_authority_hash:
            raise RecoveryError(f"source authority manifest drift: {authority}")
    source_rows = []
