...
- 要求 Gemma atomization 的 provider context 至少為 32,768 tokens（shipped default 仍為 32,768）；12,000 estimated input tokens、2,048 output tokens、48 KiB argv、300 秒/chunk、每 chunk 最多 2 次與 parallelism 1 維持固定，較大的 provider context 不得放寬這些界線；按 fragment 原順序分批，單一過大 fragment 穩定分段且禁止截尾，強制 Copilot/Gemma 零工具 argv。
