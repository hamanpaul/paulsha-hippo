...
- **THEN** the soak count SHALL remain unchanged and the window SHALL remain open, unless more than seven calendar days have passed since the last qualifying cycle, in which case the window expires and the count restarts at the next qualifying cycle

#### Scenario: Regression or redeploy ends the soak window
- **WHEN** an executed scheduled cycle reports a non-`ok` status or a regression marker, or the deployed candidate build changes
- **THEN** the soak count SHALL reset to zero and a new window SHALL begin at the next executed cycle
