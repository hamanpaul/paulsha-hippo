# stage2-memory-usage-feedback Specification

## Purpose
規範 memory 消費端的 usage 回饋迴路：以 `runtime/ledger/offered.jsonl` 與
`runtime/ledger/memory_usage.jsonl` 為唯一來源，界定 offered → read → applied
訊號的採計條件、streaming 與 fail-soft 邊界、invalid row 的有界診斷，以及
usage 訊號回饋至 relevance/decay 的計分方式。來源為 issue #41。
## Requirements
