...
### Added
- `paulsha_hippo.lib.idle.has_mem_headroom()` 與 `_read_meminfo()`：新增以 `MemAvailable / MemTotal` 嚴格大於門檻判定記憶體 headroom 的述詞；讀不到 `/proc/meminfo`、欄位缺失或總量非正值時採 fail-safe 放行，供後續 dream resource governance gate 使用。
