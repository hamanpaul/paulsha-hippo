...
**Goal:** 把蒸餾 backend 從三個硬編碼檔位升級為宣告式 preset registry：codex/copilot/gemini headless 依本機實測 argv 接線、antigravity 誠實標 unavailable；`init --backend` 選單化、`doctor` per-preset probe、mock 情境矩陣＋真蒸餾 smoke＋openai-compatible integration smoke＋supervise 無 systemd E2E 全鏈補齊，#10 依實證條件式關單。

**Architecture:** 新增 `paulsha_hippo/backends.py` 作為 preset 單一真源（跨批次契約 7），`ops.py` 的 `_BACKENDS`／`run_init`／`run_doctor` 改為 registry 驅動；四個 argv preset 全部是既有 custom-argv 機制（`AgentExecClient`：prompt 走 stdin、stdout 取回輸出）的宣告式包裝，機制零新增。驗證分四層：registry/probe 單元測試 → mock 情境矩陣（散文包 JSON／截斷／non-zero／timeout）→ 真蒸餾 smoke（env gate ×4 preset ＋ openai-compatible 真端點）→ `dream supervise --once` 無 systemd E2E。
...
| `gemini` | ✓（node script） | ✗ rc=41：本機認證選了 vertex-ai 但無對應 env（`GOOGLE_CLOUD_PROJECT`/`GOOGLE_API_KEY`）。argv 依 `--help` 契約定案：`-p` 文字「Appended to input on stdin (if any)」 | `--version` rc=0；smoke 將 skip 並回報 |
...
class RegistryContractTests(unittest.TestCase):
    ...
    def test_antigravity_marked_unavailable(self):
        preset = backends.PRESETS["antigravity-headless"]
        self.assertFalse(preset.available)
        self.assertEqual(preset.argv_template, [])

    def test_argv_presets_use_stdin_mechanism(self):
        for name in ("claude-headless", "codex-headless",
                     "copilot-headless", "gemini-headless"):
    ...
    def test_verified_argv_templates(self):
        ...
        self.assertEqual(backends.PRESETS["copilot-headless"].argv_template,
                         ["copilot", "-s", "--no-color"])
        self.assertEqual(backends.PRESETS["gemini-headless"].argv_template,
                         ["gemini", "-p", "執行 stdin 提供的任務指示"])
...
class ProbeTests(unittest.TestCase):
    ...
    def test_probe_failure_reports_rc_and_stderr(self):
        with TemporaryDirectory() as tmp:
            bin_dir = Path(tmp)
            self._make_bin(bin_dir, "gemini", "#!/bin/sh\necho auth broken >&2\nexit 41\n")
            result = backends.probe_preset(
                backends.PRESETS["gemini-headless"],
                env={"PATH": f"{bin_dir}:/usr/bin:/bin", "HOME": tmp})
    ...
    def test_probe_unavailable_and_config_driven_short_circuit(self):
        anti = backends.probe_preset(backends.PRESETS["antigravity-headless"])
        self.assertFalse(anti.available)
        self.assertIsNone(anti.ok)
        self.assertIn("unavailable", anti.detail)
...
- copilot：stdin 為唯一 prompt 來源；帶非空 `-p` 時 stdin 注入不可靠（實測
  內容丟失），故不使用 `-p`。
- gemini：`-p` 文字依 `--help` 契約「Appended to input on stdin (if any)」，
  stdin 承載完整蒸餾 prompt、`-p` 只放收尾指示。
...
PRESETS: dict[str, BackendPreset] = {
    ...
    "gemini-headless": BackendPreset(
        ...
        argv_template=["gemini", "-p", "執行 stdin 提供的任務指示"],
        required_executable="gemini",
        doctor_probe=["gemini", "--version"],
        capabilities=frozenset({"argv-stdin"}),
    ),
...
git commit -m "feat(backends): preset registry（契約 7）＋ service-effective probe——codex/copilot/gemini 實測 argv、antigravity 標 unavailable"
```
...
- Produces: `ops._BACKENDS == tuple(backends.PRESETS)`（契約 7 向後相容面）；CLI `hippo init --backend` choices＝registry 全 keys（含 antigravity-headless，執行期以 rc 2 拒絕）；help 文字對 unavailable preset 標「（尚不可用）」
...
class BackendRegistryWiringTests(unittest.TestCase):
    ...
    def test_init_rejects_unavailable_preset(self):
        rc = ops.run_init(memory_root=None, backend="antigravity-headless",
                          base_url=None, api_key_env=None, model=None, assume_yes=True)
        self.assertEqual(rc, 2)
...
git commit -m "feat(ops): _BACKENDS 改由 registry 導出；init --backend 選單化——antigravity 顯示但不可選"
```
...
預期（2026-07-10 本機）：`claude-headless` ✓（原生執行檔，service PATH 外但 probe 可跑，detail 帶「不在 service PATH」註記）；`codex-headless`／`gemini-headless` ✗（node-shebang 在 service PATH 無 node → probe rc!=0）或 ✓ 視主機 node 安裝而定；`copilot-headless` 視 `~/.local/bin` 是否在 service PATH；`antigravity-headless` ✗ unavailable。**輸出如實貼進 PR body 佐證「doctor 對每個 preset 給出正確可用性判定」。**
...
```python
#!/usr/bin/env python3
"""散文包 JSON mock backend：模擬 codex/gemini 類 CLI 在 JSON 前後夾敘事文字。"""
from __future__ import annotations
...
### Task 7: 真蒸餾 smoke ×4 preset（env gate＋skip 回報制）

**Files:**
- Modify: `tests/test_atomizer_llm_live.py:1-52`（保留既有 gemma4 live 測試，新增矩陣 class）

**Interfaces:**
- Consumes: `backends.PRESETS`／`backends.probe_preset()`（Task 1）；`AgentExecClient`／`AgentExecError`（既有）；`run_static_gate_check_file`（既有）
- Produces: env gate `PSC_ATOMIZE_LIVE=1` 下的 4 個 live 測試（`test_live_claude_headless` 等）；每 preset 證據行（stderr JSON：`{"smoke":"atomize-live","backend":...,"argv":[...],"slices":N}`）；不可用 preset → `skipTest` 帶原因（風險表：不擋批次、影響 #10 關單）
...
class AtomizerLlmLiveMatrixTests(unittest.TestCase):
    """spec §3.5.5 真蒸餾 smoke：同一 fixture session × 每個本機可用 preset。

    happy path（純 JSON 情境）一輪；其餘四情境見 mock 矩陣
    （tests/test_atomizer_backend_matrix.py）。preset 不可用（未安裝／launch
    probe 失敗，如 gemini 認證未備）→ skip 並回報原因——不擋批次，但影響
    #10 關單條件（spec §3.5 關單條件、§8 風險表）。
    """
    ...
    def test_live_gemini_headless(self):
        self._smoke("gemini-headless")
...
- [ ] **Step 2: 無 gate 跑——確認被 skip（CI 安全）**

執行：`python3 -m pytest tests/test_atomizer_llm_live.py -v -ra`
預期：5 tests 全 SKIPPED（reason: `set PSC_ATOMIZE_LIVE=1 ...`）。

- [ ] **Step 3: 帶 gate 真跑 ×4 preset（驗收證據，約 5–15 分鐘）**
...
test_live_gemini_headless SKIPPED（gemini-headless 本機不可用（launch probe）：agent exited with code 41）
```

skip/fail 以實際輸出為準——**不得竄改測試遷就結果**；gemini 若使用者已修 auth 則應 PASS。把整段輸出（含證據行與 skip 原因）存進 PR body。若某 preset 因暫時性配額失敗，重跑該單測一次（風險表：smoke 標記可重試）。

- [ ] **Step 4: Commit**

```bash
git add tests/test_atomizer_llm_live.py
git commit -m "test(atomizer): 真蒸餾 smoke ×4 preset——PSC_ATOMIZE_LIVE gate、雙層 probe、不可用轉 skip 回報"
```
...
預期：端點在線 → PASSED＋stderr 證據行；`$PSC_CLAUDE_GEMMA4_UPSTREAM_URL` 未設或端點離線 → 如實記錄「openai-compatible 真端點證據缺」，進 Task 9 的條件式 Closes 判定（改 `Refs #10`）。
...
### Task 9: docs 同步 ＋ changelog.d 碎片 ＋ CHANGELOG `[Unreleased]` ＋ 全套驗證 ＋ 條件式 Closes #10 PR 說明
...
- Produces: #10 關單判定材料（條件式 Closes 規則落在 PR body）；R-18 docs 同步完成；R-09 CHANGELOG `[Unreleased]` entry＋changelog.d 碎片（兩者並存——`[Unreleased]` 供 R-09 gate，碎片供 release 彙整）
...
| `gemini-headless` | `gemini -p "執行 stdin 提供的任務指示"` | `gemini --version` | 認證可用（oauth-personal，或 vertex-ai＋`GOOGLE_CLOUD_PROJECT`/`GOOGLE_API_KEY` env） | ⚠ argv 依 `--help` 契約（`-p` 附加於 stdin 之後）；撰寫時本機 auth 未備（selectedType=vertex-ai 無對應 env → rc=41），smoke 會 skip 並回報 |
...
## smoke 執行方式
    ...
    # 四 preset 真蒸餾（本機可用者跑、不可用者 skip 並回報原因）
...
```markdown
> 已驗環境：WSL2＋systemd＋claude-headless。backend preset 矩陣（codex/copilot/gemini headless、openai-compatible）見 `docs/backend-matrix.md` 與 `hippo doctor` 的 preset probe；無 systemd 主機用 `hippo dream supervise`（追蹤 [#10](https://github.com/hamanpaul/paulsha-hippo/issues/10)）。
```
...
```markdown
蒸餾 backend presets：`claude-headless`（預設，零 key 管理）／`codex-headless`／`copilot-headless`／`gemini-headless`／`openai-compatible`（ollama、vLLM、內網端點）／`custom-argv`；`antigravity-headless` 尚不可用（命令契約未確認）。矩陣與前置條件見 `docs/backend-matrix.md`。
```

**合併規則（README 跨批次共用錨行；PR-A Task 12 Step 2／PR-B Task 6 Step 7／PR-C Task 7 Steps 1-2／PR-F Task 7 Step 3 帶同一條規則）**：若上列任一錨行已被 sibling 批次改寫（rebase 後與引用的 old 內容不符），不得整行覆蓋——以當前行內容為基底，僅追加／改寫本批負責的片段，保留 sibling 已 merge 的全部新增。本批片段逐行為：（a）僅把「其他 backend（codex/copilot/openai-compatible headless）與無 systemd 主機為 opt-in，見 #10」這段敘述改寫為上述 preset 矩陣＋`hippo dream supervise` 指引，行內 sibling 新增的其他敘述原樣保留；（b）僅在 `hippo dream supervise` 前景模式之後追加「（`--once` 可單輪驗收）」；（c）僅把三檔位清單擴充為 preset 全清單並追加 `antigravity-headless` 尚不可用註記與「矩陣與前置條件見 `docs/backend-matrix.md`」；sibling 已 merge 的命令與後續補充行一律原樣保留，不得覆蓋或刪除。
...
```markdown
### Added
- Backend preset registry（`paulsha_hippo/backends.py`，契約 7）：`claude/codex/copilot/gemini-headless` 四個 argv presets（custom-argv 機制包裝、本機實測 argv、機制零新增）＋`antigravity-headless` 標 unavailable；`hippo init --backend` 選單由 registry 驅動、寫入時 argv[0] 絕對路徑化（fail-closed 不落半套 config）；`hippo doctor` 新增 per-preset probe 報告（service-effective 環境，能暴露 node-shebang 類 service PATH 故障）。
- `hippo dream supervise` 新增 `--once`／`--max-load`／`--promoter`／`--agent-command`：無 systemd 主機可前台單輪驗收（#10 原始 checklist 項）。
- 測試矩陣：mock 情境 ×4（散文包 JSON／截斷／non-zero／timeout→promoted/parked(invalid_output)/transient）、真蒸餾 smoke ×4 preset（`PSC_ATOMIZE_LIVE` gate、不可用轉 skip 回報）、openai-compatible 真端點 integration smoke（`HIPPO_SMOKE_OPENAI_*` gate）、supervise 無 systemd E2E。

### Docs
- 新增 `docs/backend-matrix.md`（preset argv 契約／probe／前置條件／實測狀態）；README backend 段同步（R-18）。
```
...
- init --backend 選單化（antigravity 顯示不可選）；argv[0] 絕對路徑化（沿 PR-A resolve_backend_argv）
- doctor per-preset probe（service-effective 環境）
- mock 情境矩陣 ×4＋真蒸餾 smoke ×4 preset＋openai-compatible integration smoke＋supervise --once 無 systemd E2E
...
- 真蒸餾 smoke（PSC_ATOMIZE_LIVE=1）：claude ＿／codex ＿／copilot ＿／gemini ＿（pass/skip＋原因）
- openai-compatible 真端點：＿（pass／證據缺原因）

## 關單判定（spec §3.5 關單條件，二選一）
- [ ] 「四 preset smoke＋supervise E2E＋openai-compatible 真端點」全部有實證
      → 本 PR 用 `Closes #10`
- [ ] 任一缺（如 gemini auth 未備／無 openai 端點）→ 本 PR 用 `Refs #10`
      ＋ label `policy-exempt:issue-link`，缺項列於下方、收口批次拆新 issue 後才關 #10：
      - 缺項：＿＿
...
- 四 preset 全 PASS 且 openai smoke PASS 且 supervise E2E PASS → `Closes #10`。
- 2026-07-10 基線預期：gemini skip（auth）→ 走 `Refs #10`＋`policy-exempt:issue-link`，缺項寫「gemini-headless 真蒸餾實證（本機 auth 未備）」＋（若適用）「openai-compatible 真端點實證」。
...
- **Spec §3.5 覆蓋對照**：(1) declarative registry → Task 1；(2) 三新 preset＋antigravity unavailable → Task 1/2；(3) init 選單化＋絕對路徑 → Task 2/3；(4) doctor per-preset probe → Task 4；(5) 真蒸餾 smoke＋四 mock 情境 → Task 5/7；(6) openai-compatible integration profile → Task 8；(7) supervise E2E → Task 6；關單條件 → Task 9。驗收四項（smoke 證據／mock 全過／doctor 判定／supervise E2E）各有對應 Step。
