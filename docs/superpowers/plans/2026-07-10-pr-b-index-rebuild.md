...
- **掃描檔案唯一去向（分割規則；census 以刻意雙寫的獨立實作對齊——分支順序與 reason 字串一字不差，但不共用分類代碼，見 Task 6「刻意雙寫」注記）**：
...
**Interfaces:**
- Consumes: Task 5 的 `search.coverage_path()`、`search.index_path()`、`search.SearchIndexError`、coverage 六鍵 dict；與 indexing 路徑共用的**只有原語**——`fio.read`（低階 frontmatter 解析）、`os.walk`／`instruction_corpus.discover_instruction_docs`（檔案枚舉）、sqlite 查詢、`load_projects_config`（scoping 設定讀取）。**不得 import `noise.py` 的生產 classifier（`pool_exclude_reason`／`classify_noise`／`is_generic_title`）與語料建構 helper（`instruction_corpus.corpus_for_roots`／`noise.build_corpus`）**——pool／noise 分類規則於 census 內獨立雙寫（見下方注記）
- Produces:
...
> **刻意雙寫（防同源自證——spec §3.2「獨立 filesystem census 與 index 掃描分離實作」）**：census 與 `build_index` 的分類邏輯**刻意雙寫**——pool／noise eligibility 規則在 `census.py` 內重寫一份，即使與生產版（`noise.py`）語義相同也照寫；兩邊只允許共用檔案枚舉、`fio.read` 低階解析、sqlite 查詢等原語。若 census 重用生產 classifier，classifier 本身的 bug 會同時複製到 indexing 與 census 兩邊，`eligible == indexed` 照樣通過（同源自證復活）。**兩邊規則若有意圖內的變更，必須同步改兩份，且測試會抓不一致**——漏改任何一邊時 `reconcile_index` 的分佈比對顯性回報；`ClassifierDivergenceTests` 另以「生產 classifier 注入 bug」直接驗證 census 抓得到 divergence，並以原始碼靜態檢查鎖住 import 面。
...
```bash
python3 -m pytest tests/test_moc_census.py::CensusTests tests/test_moc_census.py::ClassifierDivergenceTests -v
```
Expected: `CensusTests` 5 個 + `ClassifierDivergenceTests` 3 個全 PASS（含「生產 classifier 注入 bug、census 抓到 divergence」兩案與原始碼靜態隔離檢查）；`IndexVerifyCliTests` 仍 FAIL（`SystemExit: 2`，子命令未接線）。
...
| 三方對賬（防同源自證；census 分類規則刻意雙寫、DB 反查、互斥完備分割） | `CensusTests` 全部 + `ClassifierDivergenceTests`（生產 classifier 注入 bug 必被 census 抓到＋census 原始碼靜態隔離）+ `IndexVerifyCliTests` |
