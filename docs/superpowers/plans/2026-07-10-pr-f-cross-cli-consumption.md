...
Task 2..5 → Task 7（文件漂移）→ Task 8（Claude 真實 E2E）→ Task 9（收尾/retitle/PR 準備）
```
...
（Task 8 執行 `tests/cross_cli_live_check.sh` 後，貼入去識別 ledger 輸出：
...
**Interfaces:**
- Consumes: 契約 8 schema 定義；`sanitize_id`／`log_warn`（既有）
- Produces:
  - `hippo usage mark-applied --memory-root <path> --session-id <id> --slice-id <sl-id> --tool <name>` → append `{"kind":"applied","session_id":...,"slice_id":...,"tool":...,"ts":...}` 至 `runtime/ledger/memory_usage.jsonl`，stdout 回顯該事件 JSON，exit 0
...
```python
# tests/test_usage_mark_applied.py — applied 顯式訊號（契約 8）
import json
...
def test_mark_applied_appends_contract_event(tmp_path, capsys):
    rc = cli.main(["usage", "mark-applied", "--memory-root", str(tmp_path),
                   ...
                   "--tool", "claude-code"])
    assert rc == 0
    lines = (tmp_path / "runtime" / "ledger" / "memory_usage.jsonl") \
...
def test_mark_applied_appends_not_truncates(tmp_path, capsys):
    ...
    assert rc == 0
    lines = (led / "memory_usage.jsonl").read_text(encoding="utf-8").splitlines()
...
預期：`test_mark_applied_*` 兩個 FAIL `assert 2 == 0`（argparse `unrecognized arguments: mark-applied ...`）；`test_usage_without_memory_root_errors_exit2` 目前 PASS（argparse required 也回 2）；`test_usage_report_still_works_without_subcommand` PASS。紅燈依據為前兩個。
...
（`datetime` / `timezone` 已於 cli.py 第 8 行 import，直接可用。）
...
`--tool` 皆必填）作為 applied 顯式訊號入口（agent structured acknowledgement），append
事件 `{"kind":"applied","session_id","slice_id","tool","ts"}` 至
...
### Task 8: 真實 adapter E2E（Claude）——offered → read + negative control + applied 實證

**目的**：#18 關單條件的兩項實證（spec §3.6）：(1) 真實 adapter E2E——**平台注入**（非手動 recall）的 `offered → read` 記錄，可綁定 session、附 negative control；(2) applied 顯式訊號至少在 Claude 平台一條實證。

**Files:**
- Create: `tests/cross_cli_live_check.sh`
- Modify: `docs/cross-cli-capability-matrix.md`（「offered → read → applied 實證」節貼入去識別證據）

**Interfaces:**
- Consumes: 本機已登入的 `claude` CLI；Task 2-5 全部落地（shortlist 尾行的 mark-applied 指引、recall CLI）；`paulsha_hippo/hooks/claude_user_prompt_submit.py`、`paulsha_hippo/hooks/claude_post_tool_use.py`（既有）
- Produces: 去識別實證輸出（matrix doc + PR body 引用）；Task 9 依「applied 實證成敗」決定 `Closes #18` 與否

- [ ] **Step 1: 確認 claude CLI 旗標形式**
...
```bash
#!/usr/bin/env bash
# cross_cli_live_check.sh — #18 關單實證（PR-F Task 8）
...
if grep -Fq '"kind": "applied"' "$USAGE" && grep -Fq '"slice_id": "sl-e2e0000000000001"' "$USAGE"; then
  ...
  echo "       仍無 → PR 僅 Closes #17，#18 留 open 並留言記錄（spec §3.6 關單條件）"
fi
...
預期：`offered（平台注入）OK`、`offered→read 同 session 綁定 OK`、`negative control OK`、`applied 實證 OK`（或 WARN）。applied WARN 時整支重跑，**上限 2 次**；仍 WARN → 記錄於 matrix 與 PR body，Task 9 走「僅 Closes #17」分支。

- [ ] **Step 4: 證據落 matrix 文件**
...
git commit -m "test(e2e): Claude 真實 adapter offered→read＋negative control＋applied 實證腳本與證據（#18）"
```
...
**Interfaces:**
- Consumes: Task 1-8 全部產出；Task 8 的 applied 實證判定
...
- `hippo usage mark-applied`：applied 顯式訊號（agent structured acknowledgement）——ledger 事件 `{"kind":"applied","session_id","slice_id","tool","ts"}`；shortlist 尾行注入回報指引（#18）。
...
依 Task 8 判定二選一（**不得在證據不齊時帶 `Closes #18`**）：

- **applied 實證 OK 且 offered→read E2E OK**：PR body 含 `Closes #17` 與 `Closes #18`，附三段證據連結（capability matrix、E2E 去識別輸出、usage per-tool 報表範例）。
- **applied 實證缺**：PR body 僅 `Closes #17`＋`Refs #18`；另 `gh issue comment 18` 記錄已交付部分（offered→read E2E、mark-applied 介面、per-tool 報表）與剩餘缺口（applied 平台實證）。
...
| #18：真實 adapter E2E offered→read（可綁 session）＋negative control | Task 8（手動 recall 不算——E2E 用 Claude UserPromptSubmit 平台注入） |
| #18：applied 介面交付＋Claude 平台實證 | Task 3（介面）、Task 8（實證）、Task 9（條件式關單） |
