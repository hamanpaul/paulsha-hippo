...
|---|---|---|---|---|---|
| 1 | `claude` | judge、reasoner / atomization、title | `claude-sonnet` / `high` | `claude --model {MODEL} --effort {EFFORT} --print` | 可配置，service-effective probe 待 live |
| 1 | `codex` | judge、reasoner / atomization、title | `gpt-5` / `high` | `codex exec --model {MODEL} --reasoning-effort {EFFORT} --sandbox read-only --skip-git-repo-check -` | 可配置，service-effective probe 待 live |
| 2 | `agy` | fast、responsive / atomization、title | `default` / `medium` | `agy --model {MODEL} --effort {EFFORT} --headless --stdin` | 可配置，service-effective probe 待 live |
| 2 | `cg` | heavy-implementation、fast / atomization、title | `default` / `high` | `cg --model {MODEL} --effort {EFFORT} --headless --stdin` | 可配置，service-effective probe 待 live |
| 3 | `co-gem` | low-cost、fallback / atomization、title | `local` / `low` | `co-gem --model {MODEL} --effort {EFFORT} --headless --stdin` | 可配置，local fallback smoke 待 live |
| 3 | `claude-gem` | low-cost、fallback / atomization、title | `local` / `low` | `claude-gem --model {MODEL} --effort {EFFORT} --headless --stdin` | 可配置，local fallback smoke 待 live |
