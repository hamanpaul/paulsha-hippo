...
> 實查日期：2026-07-11。probe 腳本：`tests/cross_cli_probe_check.sh`；
> 證據為當日本機輸出（路徑已以 `<tmp>` / `<repo>` / `~` 去識別）。
> 判定規則：`supported` = 官方文件列出該事件 **且** 本機 probe FIRED；僅其一 = `inconclusive`（保守處理，等同不支援，不接線）。
> `read attribution` 依 Task 1 Step 3 僅以官方文件判定（不強求 fire 實測）。
...
| prompt-time shortlist（自動） | supported（UserPromptSubmit，既有佈署） | inconclusive（文件列 `UserPromptSubmit`；headless probe 未能 fire） | inconclusive（文件列 `userPromptSubmitted` / `UserPromptSubmit`；plan probe 的 `userPromptSubmit` 未 fire） |
| read attribution | supported（PostToolUse(Read)，既有佈署） | not-supported（`PostToolUse` 僅涵蓋 Bash / `apply_patch` / MCP；非 `Read` 等價） | supported（`postToolUse` 支援 `toolName` matcher；tool 表列 `view` = Read file contents） |
...
| 總評 | full | recall-capable | recall-capable |
...
- 官方文件依據：
  ...
  - 同頁 payload 區段以 `userPromptSubmitted` / `UserPromptSubmit` 為 prompt 事件名；本 task probe 依 plan 使用的 `userPromptSubmit` 並非該頁記載的 camelCase key。
...
Changes    +0 -0
Requests   0 Premium (17s)
Tokens     ↑ 57.2k • ↓ 307 (279 reasoning)
Resume     copilot --resume=44f7ffe5-d150-4750-babd-ce84c75ab922
[probe] copilot sessionStart（對照）: FIRED（支援）
[probe] copilot prompt-time hook: NOT-FIRED（對照組有 fire → 判定不支援）
